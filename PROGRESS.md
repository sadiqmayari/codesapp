...
**Shopify no-email customer link fix (2026-06-01, latest):** **Backend-only, standard redeploy — NO migration, NO `npm install`.** Orders from buyers with no email weren't getting a linked Shopify customer (email orders worked). Cause: `searchCustomer` queried `phone:+92…` unquoted → Shopify misparses the leading `+` → returning customer not found → `createCustomer` then hit "phone has already been taken" → best-effort catch returned null (no link). Fix: quote search terms (`phone:"+92…"`/`email:"…"`) AND in `findOrCreateCustomer` re-search-and-link when create fails because the customer already exists. New phone-only customers still create; returning phone-only customers now link. Still best-effort (never blocks the order). See ERRORS "[Shopify] Order didn't create/link a customer when the buyer had no email".
