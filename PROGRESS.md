...
- **24h recency cap (`a2435ec1`):** `loadTranscript` now time-bounds the **autonomous** context to the last 24h (combined with `cleared_before`, `CONTEXT_WINDOW_HOURS=24`) — stale old-order bleed gone. Human copilot (suggest/summarize) keeps message-cap only (no time cut).
- **WhatsApp reactions (`197ee568`):** inbound reactions render as a **badge on the target bubble** (not a new bubble); the AI never replies to stickers OR reactions (`bot-engine` early-return + `meta-webhook` `handleReaction` — no message row, no unread bump).
- **Per-chat mute restored (`95bde3fe`):** a human can mute/resume AI on a single chat even while workspace "answer all chats" is on (`effectiveOn` logic in `ai-copilot.tsx` + `conversations.ai_autoreply` override).
- **Chat closing + anti-repeat (`aeed80c0`):** a customer sign-off ("ok thanks") → AI sends ONE sign-off, resolves the conversation, stamps `ai_closed_at`, then goes silent (no more replying forever); global Jaccard anti-repeat net blocks near-duplicate replies.
- **No-fake-order + deterministic backstop (`4dbb3320`, `3c282da2`):** the model used to narrate "order placed successfully" without calling `create_order` → no order. Now (1) a no-fake-order guard blocks any "placed" claim unless an order was really created (else honest message + handoff), and (2) order summarisation + COD creation are **system-driven** (`runDeterministicOrder` from structured `AiService.draftOrder`, shared `placeCodOrder` with the same signature/window guard) — orders get created reliably regardless of whether the model calls the tool; the model still drives the conversation.
- **Status:** backend type-clean, jest **77/77**; frontend rebuilt + `sync:web`; `dist`+`dist/web` committed. **Open (go-live):** redeploy WITH `npm install` + **hPanel restart** so all of `fdb5aaab`..`3c282da2` run together; ensure both Settings → AI auto-order toggles are ON; tenant KB must hold bank details / delivery charges / >10k advance-payment policy (AI reads, never invents). Then re-test COD happy-path + reorder + prepaid + closing.

**AI Phase 2 (auto-responder) + Anthropic⇄OpenAI provider toggle (2026-06-02, latest):** **Schema change → apply migration `20260604000000_ai_autoreply` (one-time phpMyAdmin Import) + redeploy WITH `npm install`** (new `openai` dep + Prisma regen). New env `OPENAI_API_KEY` (set the key for whichever provider is active).
- **Provider abstraction:** `LlmService` gateway + `AnthropicProvider`/`OpenAiProvider` (`modules/ai/providers/`). Active backend = `platform_settings.ai_provider` (super-admin → **Platform settings** has an AI-provider radio). `PROVIDER_MODELS` (`ai.constants.ts`) holds per-provider model ids + per-token micro prices; metering prices by resolved provider+tier. Gateway 503s only if the ACTIVE provider's key is missing. OpenAI tiers: `gpt-4o-mini` (fast) / `gpt-4o` (smart).
- **Phase 2 auto-responder (fully automated, confidence-gated):** per-company `companies.ai_autoreply_enabled` (Settings → AI toggle). Orchestration in **BotsModule** (`AiAutoReplyService`) via a new **`ai` job queue** (concurrency 2) — keeps the slow model call off the `message` worker AND avoids an AiModule→InboxModule cycle. `BotEngineService.runForMessage` enqueues when inbound + no keyword bot replied + autoreply on + no human assigned; also a new **`ai_reply` bot action**. Worker: `AiService.autoReplyDecision` → structured `{handoff,reply,reason}` (fail-safe to handoff) → send via `InboxService.sendMessage` (24h window enforced; 403 → handoff) → **handoff = conversation `pending` + `needs-human` label**. Metered as feature `autoreply`.
- **Frontend:** Settings → AI gets the auto-reply toggle; super-admin Platform settings gets the provider radio; bot form gets the `ai_reply` action (with a note).
- **Status:** backend type-clean, jest **77/77**; `next build` clean; `sync:web` run; `dist`+`dist/web` rebuilt. **Open:** apply the migration + redeploy WITH `npm install` + ensure the active provider's key is set. Not hand-tested against a live provider yet.

**AI Copilot — Phase 1 (suggest-only) shipped (2026-06-02, latest):** **Schema change → apply migration `20260603000000_ai_copilot` (one-time phpMyAdmin Import) + redeploy WITH `npm install`** (Prisma client regen + new `@anthropic-ai/sdk` dep — else 5xx). New env `ANTHROPIC_API_KEY` (platform key; AI returns 503 until set, rest of app unaffected). World-class grounded AI assistant for agents, **human-in-the-loop (nothing auto-sends)**:
- **New `modules/ai/`** (`AuthGuard('jwt')+TenantGuard`): `POST /api/ai/{suggest-reply,summarize,rewrite,translate}`, `GET /api/ai/usage`, KB CRUD `/api/ai/knowledge`, settings `GET/PATCH /api/ai/settings` (PATCH owner/admin). `AnthropicClientService` (only `@anthropic-ai/sdk` caller, 503 if no key) — Haiku for suggest/rewrite/translate, Sonnet for summarize, prompt-caching on the KB block. `AiService.run` loads tenant-scoped context (last 25 msgs respecting `cleared_before` + contact + enabled KB) → **never holds a DB connection across the model call** → per-company in-process concurrency cap 3 (429) → records usage. Grounded on conversation + contact + the tenant **knowledge base** (`ai_knowledge_base`); brand-voice + default-language per company.
- **Post-paid metered billing.** `AiMeteringService` writes a per-call ledger (`ai_usage_log`) + monthly rollup (`usage_metering.ai_*`). Cost stored RAW in **micro-dollars**; markup `platform_settings.ai_price_multiplier` (seeded **1.5**) applied at billing. `InvoiceGeneratorService` folds the **previous** 30-day cycle's AI cost ×markup into the cycle invoice (`amount` + `plan_snapshot.ai_usage` + description); cycle 0 has none.
- **Gating:** effective = `subscription.ai_enabled && companies.ai_enabled`, then monthly spend cap (`companies.ai_monthly_cap_cents ?? platform default`, 0 = unlimited) → 403. `subscriptions.ai_enabled` toggled in super-admin Plans editor (whitelisted in `mapPlanData` + table "AI" pill). `GET /billing/subscription` now returns `features.aiEnabled`.
- **Frontend:** violet ✨ Copilot button in the inbox composer (`components/inbox/ai-copilot.tsx`, gated on `features.aiEnabled`) — suggest/rewrite(polite/shorten/expand/fix)/translate(English/Urdu/Roman Urdu) drop into the textarea, summarize opens a modal. Settings → **AI tab** (owner/admin): enable, brand voice, default language, self-imposed monthly cap, this-month usage card, knowledge-base editor. `lib/ai.ts` typed client.
- **Status:** backend type-clean, jest **77/77**; frontend `next build` clean; `sync:web` run; `dist`+`dist/web` rebuilt. **Open:** apply the migration on prod + redeploy WITH `npm install` + set `ANTHROPIC_API_KEY`; then flip `ai_enabled` on the desired plan(s). Phase 2 (fully-automated AI auto-responder via the bot engine) deferred. Not hand-tested against live Anthropic yet.

**Shopify order → customer link fixed via `customerByIdentifier` (2026-06-02, latest):** `923b9f3`, **backend-only, standard redeploy — NO migration, NO `npm install`.** Bug: orders created from the inbox Create-order form sometimes completed with **no customer attached** even when a customer with the same name+phone already existed in Shopify (intermittent). Root cause: lookup used `customers(query:"phone:…")` — Shopify's eventually-consistent **search index** that frequently misses an existing customer by phone; the fallback `createCustomer` then failed on the unique-phone constraint and the error was swallowed → `null` customer. Fix: new `lookupCustomerByIdentifier()` using **`customerByIdentifier`** (exact, index-free, API 2024-10+, verified live to return the customer with cost 1); `searchCustomer()` now does the exact phone-then-email identifier lookup FIRST (legacy index search kept only as a safety net — fixes both order auto-link and the modal's customer search); `findOrCreateCustomer()` recovers the existing id via the exact lookup when create is rejected for a taken phone/email instead of returning null. All best-effort — never blocks order creation. Pre-fix orders don't auto-backfill (link manually once). See ERRORS "[Shopify] Orders created from chat left with NO customer".

**Super-admin dashboard infinite-spinner fixed — slow `messages` full-scan (2026-06-01, latest):** **Backend + small frontend, standard redeploy — NO migration, NO `npm install`.** The dashboard hung (nav shown, content spinning forever) — NOT the old router-deps loader bug. Root cause: `getDashboard()` counted active-conversations-today via `SELECT COUNT(DISTINCT conversation_id) FROM messages WHERE created_at >= today`, but `messages` has no `created_at` index → full table scan; under `connection_limit=1`+`pool_timeout=0` it held the single DB connection and the request never returned (axios has no timeout). Fixed by counting from the small `conversations` table (`last_message_at >= dayStart`) instead — no `messages` scan, no DISTINCT. Safety net: `apiFetch` got an optional per-request `timeout` (not global — would break uploads/imports), dashboard uses `timeout:30000` + an **error/Retry** state so a slow query can never present as an infinite spinner again. See ERRORS "[super-admin] Dashboard spins forever".

**Dynamic pricing section on the landing page — super-admin controlled (2026-06-01, latest):** **Schema change → apply migration `20260602000000_plan_public_pricing` (one-time phpMyAdmin Import) + redeploy WITH `npm install`** (Prisma client regen for the new columns, else 5xx). Adds 8 additive/defaulted columns to `subscriptions`: `is_public BOOL`, `display_order INT`, `is_highlighted BOOL`, `tagline VARCHAR(160)`, `features JSON`, `cta_label VARCHAR(40)`, `currency VARCHAR(8) DEFAULT 'PKR'`, `billing_period VARCHAR(16) DEFAULT 'month'`. Pricing cards read straight from the real `subscriptions` rows (no price drift vs billing). New **public (unauthenticated) `GET /api/public/pricing`** (`PublicModule`, no guards, cached 5m, returns only `is_public` plans ordered by `display_order`). Super-admin **Plans editor** extended with the new fields (public toggle, highlight "Most popular", tagline, one-per-line features, currency/period, CTA label, display order) + a "Public" column; `createPlan`/`updatePlan` now whitelist fields via `mapPlanData` and bust the pricing cache. Landing `app/page.tsx` renders `components/landing/pricing-section.tsx` (fetches `/public/pricing`, **fail-graceful — hides if empty/error**, per-plan currency symbol, "+ setup" when `setup_fee>0`, CTA → `/register`). **Note:** signup → super-admin still assigns the plan on activation; the cards drive signups (CTA is generic, no plan preselect). Decisions: per-plan currency, manual feature bullets, show setup fee when >0. Backend type-clean, jest 77/77. See SCHEMA "Session Plan-public-pricing".

**Plan `webhook_enabled` flag now enforced (2026-06-01, latest):** **Backend + small frontend, standard redeploy — NO migration, NO `npm install`.** Bug: unticking Webhooks on a plan did nothing — tenants could still create/test webhooks and live events kept delivering (the flag was display-only). Fix gates the feature in two layers: `WebhookDispatcherService.isWebhookFeatureEnabled()` (cached 5m, fail-closed) short-circuits `dispatch()` so a disabled plan delivers nothing (incl. pre-existing endpoints); `WebhooksService.assertWebhookFeature()` throws 403 on create/test/retry/re-activate. `GET /billing/subscription` now additively returns `features.webhookEnabled` so the tenant `/webhooks` page shows a "not in your plan" notice instead of a bare 403. Audited sibling toggles — `webhook_enabled` was the only unenforced one (limits, usage_limit_action, overrides, Shopify order-config `enabled`/active_events all already enforced). See ERRORS "[Plans] webhook_enabled plan flag was never enforced".

**Usage/limit counter fix — contacts & templates now LIVE counts, not monthly counters (2026-06-01, latest):** **Backend-only, standard redeploy — NO migration, NO `npm install`** (no schema/Prisma change). **Bug:** at each new calendar month a tenant's contact/template "usage" reset to a tiny number (e.g. a tenant storing 2,399 contacts showed `8` with a 5,000 cap). **Root cause:** `usage_metering.contacts_stored`/`templates_used` are PER-MONTH counters (row keyed by `period=YYYY-MM`, starts at 0 each month, only ever counted "added this period"). Contacts/templates are *cumulative/stored* resources — the plan cap limits the TOTAL stored, so "current usage" must be a LIVE `COUNT`, not the monthly counter. **Fix:** new single-source helper `common/utils/usage-counts.ts` (`getStoredUsage` → live `contact.count`/`template.count`/active-`user.count`, all `deleted_at:null`). Rewired every consumer that treated the monthly counter as the cumulative current: **PlanGuard** (100% enforce — also fixes `users`, previously hardcoded `current:0`), **LimitWarningService** (80%), **LimitNotifierService** (`evaluate` 90/99/100 + banner `snapshot`), **BillingService.getSubscription** (tenant /billing), **AnalyticsService.usage** (tenant /analytics + dashboard, which reuses `usage()`), **SuperAdminService.getClientDetail** (client-profile limit bars), and **CSV-import** cap base. The **super-admin Usage page** (`getUsage`) was rebuilt company-driven: live cumulative contacts/templates AND now lists tenants with no current-month activity (previously a tenant with no current-month metering row vanished). `messages_sent`/`webhook_calls`/`conversations_opened` remain legitimate per-month consumption counters (unchanged). `usage_metering.contacts_stored`/`templates_used` are still incremented (now interpreted as "added this period") and still anchor the `thresholds_notified` fire-once ledger — no display reads them anymore. Backend type-clean; full jest suite 77/77 green. See ERRORS "[Usage] contacts/templates reset every month".

**Public marketing landing page at `/` (2026-06-01):** **Frontend-only, standard redeploy — NO migration, NO `npm install`, no new deps.** The bare domain previously redirected to `/dashboard` → login (visitors hit the login form). Now `app/page.tsx` is a full public landing: sticky nav (Sign In / Get Started, or **Go to Dashboard** when `useAuth()` has a session — adaptive, no forced redirect/flash), hero, 6-feature icon grid, a **"See it in action" features slider** (`components/landing/feature-slider.tsx` — dependency-free carousel with hand-built Tailwind UI mockups of Inbox/Broadcast/Analytics/Shopify-order using dummy data, no API/DB), a "why CodesApp" band, gradient CTA, Codentra footer. `/` is not a protected prefix in `middleware.ts` so it renders publicly; `/login`/`/register` still auto-forward authed users. (Live screenshot capture was ruled out — the backend `.env` `DATABASE_URL` is the Hostinger prod DB, not reachable/seeded locally; authed pages render from API calls so `next dev` alone shows empty shells.)

**DEFERRED — WhatsApp Business Calling API (voice) (researched 2026-06-01):** Build later (user: "we will do it later"). **Feasible on our stack without a media server** — the agent's **browser is the WebRTC endpoint** (`RTCPeerConnection`/`getUserMedia`), so media bypasses Hostinger; our `/webhooks/meta/{key}` (subscribe the **`calls`** field) + Socket.io relay handle SDP/ICE; call control = Graph **`/calls`** actions `pre_accept`→`accept`, `reject`, `terminate`, outbound `initiate`. Voice only (no video); up to 1000 concurrent; Opus. **One infra gap:** needs a **managed TURN** (Cloudflare Calls / Twilio NTS / metered / Xirsys) — can't self-host on Hostinger. **Prereqs:** enable calling (`POST {"calling":{"status":"ENABLED"}}`), WABA tier **≥2000** convos/24h, per-minute charges, outbound permission/opt-in, **no PSTN bridging** (answered in-app). **Phasing:** `MetaCallingService` + `calls` webhook + `calls` log table → Socket.io SDP/ICE relay → inbox call UI (ring/accept/decline/timer/mute/hangup) → TURN creds. Substantial multi-session build.

**Shopify Create-order modal refinements (2026-06-01, latest):** `cabe812` + `e51451c` + `8e604c9`, **frontend-only — standard redeploy**. (1) **Discounts now reflected in totals** — order summary computes per-line + order-level discounts (mirrors Shopify) and shows Subtotal → Item discounts → Order discount (input + applied −amount) → Shipping → **Total** (order discount was previously ignored in the total). Amounts show the rate currency when a shipping rate is picked. (2) **Per-item discount collapsed behind an inline `%` icon** on each item row (green when set) — click to reveal the discount input; discounted line price still shows. (3) **Section order is now: Items → Shipping → Order summary → Payment (COD/Prepaid) → Customer details → Tags → Notes** (shipping rates remain effect-driven so they recalc when the address/country are entered lower down).

**PWA install + desktop notifications (2026-06-01):** `7379b20`, **frontend + 1 additive backend field — standard redeploy, no migration/`npm install`**. (1) **Installable PWA** — `public/manifest.webmanifest` (standalone, `start_url=/dashboard`, theme `#16a34a`, `/icon.svg`) linked via root-layout `metadata.manifest` + `viewport.themeColor` + `appleWebApp`; `public/sw.js` service worker (install/activate/claim + no-op fetch for installability). (2) **WhatsApp Web-style OS notifications while the app is open** — `lib/notify.ts` (`registerServiceWorker`, permission, `showMessageNotification` coalesced per-conversation via `tag` + running "N new messages" count + inline **Reply**/**Open** actions, `clearConversationNotification`); `components/notification-gate.tsx` (SW register, routes SW→client `navigate` postMessage via `router.push`, dismissible "Enable notifications" prompt); `(app)/layout` fires an OS notification on `message.received` when the window **isn't focused** and you're not on that thread (sender name + text/media preview), and clears on open + `message.read.bulk`. (3) **Inline reply from the notification** — `sw.js` `notificationclick` handles the typed reply by minting a token via the httpOnly refresh cookie (`POST /api/auth/refresh`) then `POST /api/inbox/conversations/:id/send` (access token never stored); body/Open click focuses/opens the app to the chat. `meta-webhook` emit now includes `contactName`. **Scope note:** notifications fire only while a tab/PWA is open (no Web Push / closed-app delivery — deferred; would need VAPID + per-device subscription storage + server push).

**WhatsApp-style voice-note player (2026-06-01):** `e652f55`, **frontend-only — standard redeploy**. New `components/inbox/audio-message.tsx` replaces the default `<audio controls>` in the inbox bubble: play/pause + clickable & draggable **waveform scrubber** (deterministic pseudo-waveform per message, thumb dot, elapsed/total time; forces duration calc for ogg/opus that report `Infinity`). A **module-level audio bus** gives single-playback (starting one note pauses any other playing) + **auto-advance** to the next audio note on end. Green/white theming for outbound/inbound. Follow-up `9105d61`: auto-advance now fires ONLY when the immediately-next message is also audio (a text/image between two notes stops the chain — thread builds a `nextAudioMap` passed Bubble→AudioMessage; bus plays that exact id); waveform moved onto the play-button row so it's vertically centred.

**Inbox attachments + emoji + stickers (2026-06-01):** `b023671`, **frontend-only — standard redeploy, no migration/`npm install`** (inbound sticker download already existed in `meta-webhook.service`). (1) **Paste a screenshot/image** into the composer (`onPaste` → stages via `validateFile` + existing `/send-media`). (2) **Drag & drop** a file anywhere over the thread → full drop overlay → stages it (guarded to the 24h window + disabled mid voice-record; dragDepth counter avoids flicker). (3) **Emoji picker** — new dependency-free `components/inbox/emoji-picker.tsx` (categorized unicode grid), `Smile` button in the composer inserts at the cursor (plain text). (4) **Inbound stickers now render** — `Bubble` includes `sticker` in `isMedia` + renders the webp `media_url` as an image w/ lightbox (was previously invisible). Text emojis already rendered natively.

**Broadcast campaign builder + inbox/Shopify polish (2026-06-01):** Two commits, both **standard redeploy — NO migration, NO `npm install`** (no schema/Prisma change).
- `c696325` — (1) **Inbox search now matches message content**, not just contact name/phone (`inbox.service.listConversations` `where.OR` over contact name/phone + `messages.some.content contains`); placeholder → "Search name, phone or message". (2) **Shopify Create-order modal: Address + City are now REQUIRED** (button disabled until both filled, red field hint + inline note). (3) **Browser-autofill no longer bleeds into the chat/product search boxes** — chat search + product search are `type=search autoComplete=off` w/ explicit names; customer fields carry `autocomplete` tokens (name/tel/email/address-line1/address-level2).
- `5a02981` — **World-class broadcast campaign builder.** Backend additive (no schema): `POST /broadcasts/preview-audience` (live count + sample), `POST /broadcasts/test-send` (one real template msg, no counter touch), `POST /broadcasts/:id/duplicate` (clone→draft), **"All contacts"** audience (`audience.all` → all active contacts), and **per-recipient personalization** — variable values may be literals OR contact tokens `{{contact.name|phone|email}}` / `{{contact.custom.<key>}}`, resolved per contact at send time via shared `BroadcastsService.buildTemplateComponents` (worker + test-send). Frontend: `/broadcasts/new` rewritten as a **4-step wizard** (Details → Audience → Personalize → Review) with a sticky live WhatsApp-style `TemplatePreview`; `AudienceBuilder` extended to 4 modes incl. a searchable multi-select `ContactPicker`; personalization mapping UI + inline test-send; review step (send-now / schedule); **Clone** button on the list. New FE files: `components/broadcasts/{contact-picker,template-preview}.tsx`, `lib/broadcast-utils.ts`. New BE DTOs: `broadcasts/dto/{preview-audience,test-send}.dto.ts`.
- **Note:** webhook "test gives error" turned out to be a **404 from the user's own destination URL** (not a CodesApp bug); resolved by pointing the endpoint at a real POST handler.

---

## Prior status (Super-admin redesign batch)
...
**Super-admin redesign — Phase 2 (2026-05-23, latest):** Full client profile page at `/super-admin/clients/[id]` — replaces the modal. New backend endpoint, new frontend route, list page trimmed.
- **Backend:** new `GET /api/super-admin/clients/:id/detail` (`SuperAdminService.getClientDetail`) returns the expanded payload in ONE call — company + subscription + users + a snapshot of current-month KPIs (active contacts / templates / users / messages / convos / window-open chats / open invoices / outstanding / MRR), current `usage_metering` row, full invoice history, Shopify integration row, last 50 audit-log rows. Effective `usage_limit_action` resolved (override `??` platform default). Sibling of the existing `GET /clients/:id` (kept intact for backward compat). Decimals wrapped in `numifyDecimals`.
- **Frontend `/super-admin/clients/[id]`:** header strip (logo + name + status pill + plan badge + MRR + Activate/Suspend/Reactivate/Impersonate); 8 snapshot tiles; Owner & users table; Integrations panel (WhatsApp / Meta webhook / Shopify connection cards); Webhook callbacks card with copy-to-clipboard chips (Meta `/webhooks/meta/{key}`, Shopify `/webhooks/shopify/{key}`, both resolved from `window.location.origin` at runtime); Plan & limits with per-limit progress bars (green/amber/red); Billing & lifecycle card (activated_at / suspended_at / grace_until / effective usage policy + override select + grace-date grant/clear + **per-row "Mark paid" buttons** wired to existing `POST /api/super-admin/billing/invoices/:id/mark-paid`); Activity log (last 50, action as tinted-green code chip); **Danger zone — Minimal variant only** (Delete client with **type-the-company-name** double-confirm; see "Phase 2 Danger-zone decision DEFERRED" below — reset-pw / force-signout / wipe-WA stay deferred).
- **Clients list:** Details button → `View profile` Link to the new page. Removed the Details modal + delete-confirm + grace/usage-policy controls (all moved to profile). Net `-300` lines.
- **Open:** standard redeploy (no migration, no `npm install`). Not hand-tested against live data yet.

**Super-admin redesign — Phase 3 (2026-05-23, latest):** Invoice generator UI + per-client one-off invoices + super-admin per-row Mark Paid. One small new backend endpoint; the other two actions reuse existing routes.
- **Backend:** new `POST /api/super-admin/clients/:id/invoices` (`SuperAdminService.createOneOffInvoice`). Creates an **off-cycle** invoice with a deliberately distinct number namespace `INV-{companyId}-OFF-{YYMMDD-HHMMSS}` — never collides with the cycle invoice numbers `INV-{companyId}-{cycleStartYYYYMMDD}` that `InvoiceGeneratorService.generateDueInvoices()` raises. `status='pending'`, `period=null`, `due_date` defaults to `now + 7d` (matches the cycle). BadRequestException on amount ≤ 0. No schema change — uses the existing `invoices` columns + a nullable `description`.
- **Frontend `/super-admin/billing`:** new **Run invoice generation** button (top-right, green CTA) → confirm dialog → calls existing `POST /api/super-admin/billing/invoices/generate` (the same routine the daily cron uses; idempotent — cycle invoices that already exist are skipped). Result toasted as `Generated N · skipped M`. Per-row **Mark paid** button on every pending/overdue invoice (was view-only; reuses existing `POST /api/super-admin/billing/invoices/:id/mark-paid` which auto-reactivates a cron-suspended company once all invoices are paid).
- **Frontend `/super-admin/clients/[id]`:** **+ New invoice** button next to the Invoices header in the Billing & lifecycle card → simple modal (amount, optional description, optional due date) → posts to the new endpoint and refetches the profile. Uses the same backdrop-click + Esc-safe modal pattern as the delete-confirm.
- **Open:** standard redeploy (no migration, no `npm install`). Not hand-tested against live data yet.

**Super-admin redesign — Phases 4 + 4.5 (2026-05-23, latest):** Enterprise plan + per-client editable limit overrides + 90/99/100% usage notifications + suspension email. **ONE migration in the whole batch.**
- **Migration `20260531000000_company_overrides_and_warnings`** (one-time phpMyAdmin Import; MySQL 8, no IF NOT EXISTS — re-run fails on duplicate column). Adds: `companies.{contact_limit_override,template_limit_override,user_limit_override} INT NULL` (Phase 4 — per-client editable caps) AND `usage_metering.thresholds_notified JSON NULL` (Phase 4.5 — fire-once ledger keyed `dim:threshold`, e.g. `["contacts:90","contacts:99","templates:90"]`). Pair with redeploy **WITH `npm install`** (Prisma client regen — else 5xx on inbox/billing/super-admin routes).
- **Effective limits resolver:** `common/utils/effective-limits.ts` returns `override ?? subscription.<field>`. Used by `PlanGuard` (100% enforce), `LimitNotifierService` (90/99/100 fire), `LimitWarningService` (legacy 80% webhook), and `SuperAdminService.getClientDetail` (UI).
- **PlanGuard:** now reads `companies.{contact,template,user}_limit_override` alongside the subscription and caches the resolved EFFECTIVE numbers (5m). Cache invalidated on override change.
- **New backend:** `MailService` (`common/services/`, Resend-or-SMTP, never-throws; auth keeps its own inline sender — not migrated this commit). `LimitNotifierService` (billing module — central 90/99/100 engine; fire-once via `usage_metering.thresholds_notified`; per threshold emits socket `usage.warning`, sends owner email, fires outbound webhook `subscription.limit.warning` / `subscription.limit.reached` at 100; non-throwing). `UsageMeteringService.increment` now calls both `LimitWarningService.check` (80%) AND `LimitNotifierService.evaluate` (90/99/100) after every metered increment. **Scope cut for v1:** only `contacts` + `templates` evaluated (the two metered + capped dimensions); `users` notifications deferred (users limit is a live count, not per-period).
- **New endpoint `GET /api/billing/usage-warnings`** (`AuthGuard('jwt')+TenantGuard`) — hydration for the in-app banner + navbar pulse on page load. Returns `[{dim,threshold,pct,current,limit,severity}]` for the currently-active warnings only.
- **New endpoint `PATCH /api/super-admin/clients/:id/limits`** (`{contact_limit?,template_limit?,user_limit?}`; field-level — `null` clears that override). Validates non-negative integers; invalidates the PlanGuard cache so new caps take effect next request.
- **Suspension email:** `LimitNotifierService.sendSuspensionEmail(companyId)` — fired (a) from `SuperAdminService.suspendClient` on the non-suspended → suspended transition only (no re-spam on double-click), (b) from `BillingService.enforceCron` for every auto-suspended company per run. Owner email shows outstanding balance + `admin@codentra.pk` contact.
- **Frontend banner (`components/usage-warning-banner.tsx`):** sticky above the navbar in `(app)/layout.tsx`. Hydrates from `/billing/usage-warnings` on mount, subscribes to `usage.warning` socket for live cross. **Per-session dismissal** (sessionStorage key `${companyId}:${period}:${dim}:${threshold}`) — re-appears next session while the condition holds. Yellow (90%) static / red (99%, 100%) **pulsing** gradient; "View usage" link to `/billing`, X to dismiss. Reports highest active severity up to the shell.
- **Frontend navbar:** pulsing `AlertTriangle` chip beside the company name when severity is `critical` (≥99%); static amber chip at `warn` (90%); hidden otherwise. Clicks through to `/billing`.
- **Frontend `/super-admin/clients/[id]`:** the **Plan & limits** card now shows progress bars against EFFECTIVE limits (override ?? plan default), each bar gets a `OVR` purple chip when an override is active and a `plan default N` footnote. **Edit overrides** modal (3 numeric inputs; blank = clear override) calls the new PATCH endpoint.
- **Open:** apply the migration on prod + redeploy WITH `npm install`. Not hand-tested against live data yet.

**Timezone + PDF invoice + Legacy-invoice rewrite (2026-05-23, latest):** Three features in one commit. ONE schema change (the timezone column); the others reuse existing endpoints.
- **`companies.timezone` (migration `20260601000000_company_timezone`)** — nullable `VARCHAR(64)` for the tenant's IANA name. Frontend `lib/utils.ts` has `setActiveTimeZone(tz)` that AuthProvider calls after `/auth/me`. `fmtDate`/`fmtDateTime` now build `Intl.DateTimeFormat` with `timeZone: activeTimeZone` so the WHOLE app renders timestamps in the tenant's clock. `null` = browser default. Auth-controller `PATCH /api/auth/company/timezone {timezone:string|null}` (owner/admin only) validates the IANA name via `Intl` before writing. Apply migration + redeploy **WITH `npm install`**.
- **Beautiful printable invoice (`/billing/invoice/[id]/print`)** — dedicated route under `(app)/` (JWT-gated). Hydrates from the already-existing `GET /api/billing/invoices/:id` (extended in this commit to include company logo, owner, plan snapshot). A4-portrait print stylesheet via `@page`; "Print / Save as PDF" button uses `window.print()` and the browser produces the PDF natively — no puppeteer, no new dep. Tenant `/billing` page has a `PDF` icon-link on every invoice row + a "Download PDF" CTA inside the existing detail modal. Super-admins reach it via the existing impersonate flow.
- **Legacy-invoice rewrite script (`backend/scripts/rewrite-legacy-invoices.ts`)** — one-shot CLI for pre-billing-lifecycle invoices. Dry-run by default; `--apply` writes; `--company=N` scopes to one tenant. Recomputes `invoice_number` to `INV-{companyId}-{cycleStartYYYYMMDD}`, `period` to the cycle-start month, `due_date` to `cycleStart + 7d`, `description`, and adds `cycle_index`/`cycle_start` to `plan_snapshot`. NEVER touches `status`, `paid_at`, or `amount`. Skips invoices whose company has no `activated_at` and any that would collide with an existing canonical number. Excluded from the nest build via `tsconfig.build.json` (the `scripts/` directory was shifting nest's rootDir; without the exclude `dist/main.js` ends up at `dist/src/main.js` and `server.js` can't find it).
- **Open:** apply migration on prod + redeploy WITH `npm install`. Legacy script is opt-in (`npx ts-node scripts/rewrite-legacy-invoices.ts --apply` after confirming dry-run output).

**Super-admin redesign — Phase 5 (2026-05-24, latest):** Suspended-workflow polish. Frontend-only — no migration, no `npm install`, no new backend endpoint.
- **`components/billing-blocked.tsx`:** the screen a suspended tenant sees instead of the app shell now (a) lists each outstanding invoice with **days-overdue** computed from `due_date`, (b) shows a separated **Total due** row, and (c) renders an **"Email billing support"** CTA — a `mailto:admin@codentra.pk` link with pre-filled subject `[Suspension] {company name} — settle outstanding $N` and a body that itemizes the invoices and totals. Pulls `company.name` from `useAuth()` (already on `/auth/me`). Sign-out button kept.
- **`/super-admin/clients/[id]`:** new **Resume access** amber panel rendered ABOVE the Danger zone, **only when `activation_status==='suspended'`**. One-click action: iterate every pending/overdue invoice and call the existing `POST /api/super-admin/billing/invoices/:id/mark-paid`, then `PATCH /api/super-admin/clients/:id/activate`. Each step is independent (no transaction available across these endpoints); toast summarizes the result (`Access resumed (3 invoices settled)` / `Access resumed (2 paid, 1 failed)` / `Marked N paid — reactivation failed`). Audit entries land automatically because `markPaid` and `activateClient` already log them. Uses the existing `ConfirmDialog`. No new endpoint, no new type.
- **Open:** standard redeploy (no migration, no `npm install`). Not hand-tested against live data yet.

**Super-admin redesign — Phase 2 Danger-zone decision DEFERRED (2026-05-23):** The Phase 2 client-profile page (`/super-admin/clients/[id]`) will include a bottom-of-page "Danger zone" section. The scope of that section is **deferred** — Phase 2 will ship with the **Minimal** variant only (just the existing `DELETE /api/super-admin/clients/:id` exposed via a type-the-company-name double-confirm). Reset-owner-password / force-sign-out / wipe-WhatsApp-creds are NOT in Phase 2; they can be added later as their own phase if a need arises. **Do not implement them in Phase 2 unless the user explicitly reopens the question.**

The three deferred items (all backed by NEW endpoints + 1 schema column for force-sign-out — none of which exist today):
- **Reset owner password** — `POST /api/super-admin/clients/:id/reset-owner-password` — generates a temp password, returns it once, forces change-on-next-login.
- **Force sign-out (revoke all sessions)** — needs a `users.refresh_token_invalidated_at` (or `token_version`) column the JWT strategy checks; invalidates every refresh token for the tenant.
- **Wipe WhatsApp credentials** — `POST /api/super-admin/clients/:id/wipe-whatsapp` — nulls encrypted Meta tokens + webhook secrets so the tenant must re-onboard. Useful if a token leaks.

Out of scope entirely (do not re-propose): hard-delete data while keeping the company (no use case); GDPR anonymize/export (deserves its own page, not a danger-zone button).

**Super-admin redesign — Phase 1b (2026-05-23, latest):** Repainted the 5 remaining dark pages (`/super-admin/{billing, plans, usage, audit, settings}`) to the new light Power-BI chrome so the whole super-admin area is visually consistent. Frontend-only, no backend/schema/env change.
- **Billing:** light white-card table, lighter status pills (paid=green, pending=amber, overdue=red, cancelled=slate), title with Receipt icon chip, filter chips matching the new design tokens.
- **Plans:** white card + light table; webhook column now uses a pill; edit button is a hover-tinted icon-button; New-plan CTA gets a shadow; form inputs got `focus:ring-green-600`.
- **Usage:** light card + plan-name pill; each capped cell now shows a **slim progress bar** under the number (green/amber/red), making at-a-glance limit pressure visible per row.
- **Audit:** light card + table; user shown with name/email stack; `action` rendered as a tinted-green code chip; metadata shown in a subtle code box; title with ScrollText icon chip.
- **Settings:** white card; selected radio gets a green ring + tinted bg; Save button shadowed; success state shows the CheckCircle.
- **Open:** standard redeploy (no migration, no `npm install`).

**Super-admin redesign — Phase 1a (2026-05-23):** Visual foundation + comprehensive overview.
- **Backend:** `getDashboard()` rewritten — single endpoint returning KPIs (totalClients/active/pending/suspended/totalUsers/MRR/invoiced-this-month/paid-this-month/outstanding/new-signups/active-conversations-today), 90-day signups trend, top 5 pending approvals (incl. owner email), top 5 overdue invoices (with days-overdue), last 10 audit events. No schema change.
- **Frontend:** super-admin shell repainted to a light Power-BI-style chrome (white card layout, sticky nav, gradient brand chip, mobile-friendly horizontal nav). New Overview page: 10 colored KPI tiles, signups area chart, three side-by-side widgets (Pending approvals / Overdue invoices / Recent activity), "Today" strip with active conversations. Clients list restyled to the same light design — table with avatar column, MRR column, status filter chips with live counts, in-place Details modal preserved.
- **Other super-admin pages (billing/plans/usage/audit/settings) still on dark styling** — they will visually clash with the new light chrome until Phase 1b restyles them (next commit). Functionally unaffected.
- **Open:** standard redeploy (no migration, no `npm install`).

**Analytics rebuild — Power-BI-style dashboard (2026-05-23, latest):** Full ground-up replacement of the analytics page + a new single-call backend endpoint.
- **Backend:** new `GET /api/analytics/dashboard?from&to&granularity&compare` returning one payload with everything: range-aware KPIs (sent, received, active convos, unique contacts engaged, new contacts, delivery %, read %, reply %, **real avg first-response latency**, bot-handled %), each carrying `prev` + `deltaPct` (previous-period comparison: same window length ending at `from`), plus a time-series trend (hour or day buckets), funnel totals (sent/delivered/read/replied), conversation status breakdown, hourly heatmap (DOW×hour), agent leaderboard (using NEW `messages.user_id` for true per-agent attribution), top-10 most-active contacts, conversation cost, and current-month usage vs plan. Cache 60s (was 5 min on the old patchwork). Old `/overview /funnel /agents /conversation-cost /usage` routes left in place for backward-compat.
- **Schema:** new migration `20260530000000_message_user_id` adds `messages.user_id INT NULL` (FK to users, ON DELETE SET NULL) + index. `messages.created` in `sendMessage`/`sendMedia` now writes `user_id`. **Open: apply migration on prod + redeploy WITH `npm install`** (Prisma client regen).
- **Frontend:** new `/analytics` page — sticky filter bar (1d/7d/30d/90d/custom + compare-to-previous toggle), 10 colored KPI tiles with ↑/↓ delta chips and previous-period values, smooth gradient area trend chart, custom horizontal funnel (with stage-to-stage conversion %), conversation-status donut, day-of-week × hour-of-day **heatmap** with green-intensity scale, agent leaderboard with real per-agent attribution + real response time, most-active contacts table, cost + usage cards. Tailwind only, recharts for trend/donut. Modern card layout with subtle gradients.
- **Fixes vs old analytics:** KPI strip now respects the date range (was lifetime); avgResponseTime is now real first-response latency (was conversation lifetime); `messagesSent` per agent now comes from `messages.user_id` (was "outbound messages on conversations currently assigned to that user" — wildly misattributed); reply rate vs replied-line definitions are now consistent within the dashboard.
