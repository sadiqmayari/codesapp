...
## AI Phase 2 — auto-responder + multi-provider (2026-06-02)
- **Provider is pluggable, selected platform-wide (not per-tenant).** `LlmService` resolves `platform_settings.ai_provider` and routes to `AnthropicProvider` or `OpenAiProvider` (both implement `LlmProvider`). The result carries the resolved provider so `AiMeteringService` prices it via `PROVIDER_MODELS[provider][tier]`. Selection is a super-admin platform choice because WE bear the cost; a per-tenant choice would fragment cost/quality with no business upside in v1. Each provider has its own env key; the gateway only 503s if the ACTIVE provider's key is missing. Adding a provider = one `LlmProvider` impl + a `PROVIDER_MODELS` entry — `AiService` and metering are untouched.
- **OpenAI mapping quirks:** system blocks are concatenated into one `system` message (no explicit prompt-cache control — OpenAI auto-caches long prefixes; the `cache` flag is ignored). Usage maps `prompt_tokens - cached → inputTokens`, `cached → cacheReadTokens`, `completion_tokens → outputTokens`. Keep the `NormalizedUsage` shape stable so metering stays provider-agnostic.
- **Auto-responder lives in BotsModule, NOT AiModule — deliberate cycle avoidance.** Sending requires `InboxService`. If AiModule imported InboxModule, then `BillingModule → AiModule → InboxModule → UsageMeteringModule → BillingModule` would form the documented 4-file boot-crash cycle. So `AiModule` stays a near-leaf (exports `AiService`/`AiMeteringService`, imports only AuthModule), and the orchestration (`AiAutoReplyService`) sits in BotsModule, which already has `InboxService` via forwardRef. AiService stays pure generation+metering (no send).
- **Auto-reply runs on its own `ai` job queue (concurrency 2), not the inbound `message` worker.** A multi-second model call must not occupy a message-worker slot (only 3) nor hold the single Prisma connection. The enqueue is decoupled from the worker registration (both via the global JobQueueService), so `BotEngineService` enqueues without importing the worker — no extra module coupling.
- **Confidence-gated by construction.** `AiService.autoReplyDecision` forces a JSON `{handoff, reply, reason}` and the parser **fails safe to handoff** on any ambiguity (unparseable output, empty reply, gate 403/over-cap, or a send error like a closed 24h window). The model is system-prompted to hand off on anger/human-request/sensitive-or-unverifiable actions/low confidence. Handoff is a soft signal (conversation → `pending` + `needs-human` label); the inbound message already surfaced the chat as unread, so a missed auto-reply degrades to normal human handling, never to a dropped customer.
- **Guardrails:** never auto-replies when a human is assigned; never fires if a keyword bot already replied (prevents double-replies); only sends inside the 24h window (template-only outside → handoff). All metered/capped like interactive calls.

## AI Copilot — Phase 1 (suggest-only, grounded, metered) (2026-06-02)
- **Suggest-only by design.** Every AI output is returned to the agent to review/edit before sending — the AI never sends a WhatsApp message or mutates a conversation. This is the lowest-risk, highest-value first step; the fully-automated auto-responder (AI as a bot-engine action with confidence-gated handoff) is deferred to Phase 2 on the SAME foundation. Don't auto-send from Phase-1 endpoints.
- **One platform Anthropic key, billed back to tenants.** `ANTHROPIC_API_KEY` is the platform's (env, not per-tenant, not encrypted — it isn't a tenant secret). Cost is metered per call (RAW micro-dollars) and billed back × `platform_settings.ai_price_multiplier` (default 1.5). A per-tenant key model was rejected for v1 — the value is "it just works," and the markup is the business model. If a tenant ever wants BYO-key, add an encrypted per-company column and branch in `AnthropicClientService` — don't retrofit the metering.
- **Cost in micro-dollars, not cents.** Token costs are sub-cent; cents would round to zero. `ai_usage_log.cost_micros` + `usage_metering.ai_cost_micros` store RAW provider cost (millionths of USD); markup is applied only at billing/display (`AiMeteringService.billedCentsFor`). `ai.constants.ts` is the single source of model IDs + per-token prices (USD/MTok == micros/token).
- **Two ledgers, on purpose.** `usage_metering.ai_*` is the fast monthly rollup (cap check, dashboards); `ai_usage_log` is the authoritative per-call ledger. Post-paid billing needs the ledger because cycles are activation-anchored 30-day windows, NOT calendar months — `InvoiceGeneratorService` sums `ai_usage_log.cost_micros` over `[cycleStart(n-1), cycleStart(n))` and folds it into cycle n's invoice. Don't try to bill AI from the calendar-month rollup (window mismatch).
- **Never hold the DB connection across the model call.** With `connection_limit=1`, a multi-second AI HTTP call holding the single Prisma connection would stall the whole process. `AiService.run` loads all context first, releases, calls Anthropic (no DB), then writes metering. Interactive calls are guarded by a per-company in-process semaphore (max 3 → 429) rather than the job queue (the queue is for async/background work; copilot is synchronous/interactive). Phase-2 batch AI would use the job queue.
- **Grounding without RAG (v1).** The KB is injected whole (capped at `KB_CHAR_BUDGET`) into a prompt-cached system block — no embeddings/vector store. For the small KBs tenants start with, full-context + prompt caching is cheaper and simpler than RAG. Revisit (embeddings + retrieval) only when a tenant's enabled KB regularly exceeds the budget.
- **Gating mirrors existing patterns.** Effective AI-on = `subscription.ai_enabled && companies.ai_enabled`; the spend cap resolves `companies.ai_monthly_cap_cents ?? platform default` exactly like `usage_limit_action` / effective-limits. `features.aiEnabled` on `GET /billing/subscription` lets the UI hide AI cleanly (same as `webhookEnabled`). Plan flag is whitelisted in `mapPlanData` (the plan mass-assignment boundary) — add future AI plan fields there.

## Public pricing cards read from `subscriptions` (single source of truth) (2026-06-01)
- The landing-page pricing section is driven by the **real plan rows** (`subscriptions`), not a separate marketing table — so the advertised price/limits can never drift from what is actually billed. Marketing/visibility fields (`is_public`, `display_order`, `is_highlighted`, `tagline`, `features` JSON, `cta_label`, `currency`, `billing_period`) were added to `subscriptions` and edited in the existing super-admin Plans editor. A separate `pricing_plans` table was deliberately rejected (double entry + drift).
- `GET /api/public/pricing` (`PublicModule`) is **unauthenticated by design** — no guards, no tenant scope; it serves logged-out visitors. It lives under the `/api` prefix and returns ONLY `is_public` plans ordered by `display_order`. Cached 5m (`public:pricing` key); `SuperAdminService.createPlan/updatePlan` bust that key. NEVER put tenant/JWT data on a `public/*` route.
- `SuperAdminService.mapPlanData()` is the mass-assignment boundary for plans (the controller passes `@Body() any`) — it whitelists every editable plan field incl. the new pricing fields and coerces types. Add new plan fields there, not by widening the controller.
- The frontend pricing section (`components/landing/pricing-section.tsx`) **fails graceful**: it hides entirely on fetch error or when no plan is public, so the landing page never shows a broken/empty pricing area. Plan selection is NOT part of signup — the CTA goes to `/register` and the super-admin still assigns the plan at activation.

## Cumulative usage (contacts/templates/users) = live COUNT, not the monthly counter (2026-06-01)
- **The capped cumulative dimensions are NEVER read from `usage_metering`.** `usage_metering.contacts_stored` / `templates_used` are per-month counters (`period`-keyed, reset to 0 each calendar month). Contacts/templates/users are cumulative *stored* resources; the plan cap limits the running total. The single source of truth for "current usage of a capped dimension" is `getStoredUsage()` in `common/utils/usage-counts.ts` — live `contact.count` + `template.count` (both `deleted_at:null`, all statuses) + active `user.count`. This mirrors the `effective-limits.ts` single-source pattern.
- **Every enforcement/warning/display consumer uses it (or an inline `contact.count`/`template.count`):** `PlanGuard`, `LimitWarningService` (80%), `LimitNotifierService` (90/99/100 + banner snapshot), `BillingService.getSubscription`, `AnalyticsService.usage` (and the dashboard, which reuses `usage()`), `SuperAdminService.getClientDetail` (limit bars) and `getUsage` (the super-admin Usage page, now company-driven). DO NOT reintroduce a `usage_metering.contacts_stored`-based "current usage" anywhere — that was the month-rollover reset bug (see ERRORS "[Usage] contacts/templates reset every month").
- **What `usage_metering` is still for:** genuinely per-period consumption — `messages_sent`, `webhook_calls`, `conversations_opened` — and to host the `thresholds_notified` fire-once ledger. `contacts_stored`/`templates_used` are still incremented (now meaning "added this period") but no display reads them. Renaming those columns would be a migration and is deliberately deferred — the meaning is documented here instead.
- **Basis decision:** the cap counts ALL non-deleted contacts/templates (including `blocked`/`archived` contacts — they still occupy a stored row). `activeContacts` (non-blocked) exists only as a separate super-admin snapshot tile, not for the cap.

## Per-tenant timezone + printable invoices (2026-05-23)
- **Timezone resolution is GLOBAL in the frontend, not per-call.** `lib/utils.ts` holds a module-level `activeTimeZone`. AuthProvider calls `setActiveTimeZone(company.timezone)` after `/auth/me` returns. Every `fmtDate`/`fmtDateTime` call uses that value via `Intl.DateTimeFormat({ timeZone })`. **Don't pass tz into call sites** — there are ~120 of them and the next refactor will undo it. If you need a one-off override (e.g. an invoice PDF in the OPPOSITE tz from the viewer), build a small helper that wraps `Intl.DateTimeFormat` locally. Logout calls `setActiveTimeZone(null)` to reset to browser default for the login screen.
- **The IANA name MUST be validated** before writing — `Intl.DateTimeFormat({timeZone:'bogus'})` throws `RangeError`. The backend validates in `AuthService.updateCompanyTimezone` via a defensive `new Intl.DateTimeFormat(..)` instantiation; the frontend's `setActiveTimeZone` ALSO validates and falls back to undefined instead of crashing every render. Either side rejecting is fine; both rejecting is paranoid but cheap.
- **PDF strategy: client-side print, not server-side render.** The `/billing/invoice/[id]/print` page is a regular Next.js page sized for A4 portrait via `@page { size: A4 portrait; margin: 12mm 14mm }`. The "Print / Save as PDF" button calls `window.print()`; the browser produces the PDF. This avoids puppeteer (won't run on Hostinger shared) AND avoids pdfkit (looks ugly without ~500 lines of styling). The trade-off: the user clicks "Save as PDF" in the print dialog one extra time — acceptable for an admin/billing workflow. **Don't add a server-side PDF library** unless we move to a host that can run headless Chrome.
- **The print page is tenant-only** (under `(app)/`). Super-admin reaches it via the existing impersonate flow. Building a super-admin-direct path would require either a separate auth gate on the same route or duplicating the page — both worse than impersonation, which is a one-click action that already exists.
- **`getInvoice` returns the company + plan snapshot inline** so the print page hydrates in ONE request. The `company` shape on the response (`{ id, name, logo_url, timezone, owner: {name, email}, plan: {plan_name, monthly_price, setup_fee} }`) is the PDF's data contract — don't trim fields without checking the print page.
- **Legacy-invoice migration is opt-in.** `backend/scripts/rewrite-legacy-invoices.ts` is dry-run by default. The `scripts/` directory is EXCLUDED from `tsconfig.build.json` because including it shifts nest's rootDir computation → `dist/main.js` ends up at `dist/src/main.js` and `server.js` (which requires `./dist/main`) breaks. Don't remove that exclude.

---

## Per-client limit overrides + usage notifications (Phases 4 + 4.5 — 2026-05-23)
- The single source of truth for "what cap applies to this tenant" is `resolveEffectiveLimits()` in `common/utils/effective-limits.ts`. Every consumer (PlanGuard, LimitNotifierService, LimitWarningService, super-admin client-detail payload) uses it — DO NOT inline an `override ?? subscription.<field>` ternary in any of those callers. If you add a fourth dimension later, only this helper changes.
- `LimitNotifierService.evaluate(companyId, dim)` is wired into `UsageMeteringService.increment` AFTER the row write — same place as the legacy `LimitWarningService.check`. The two coexist (warning fires the 80% webhook; notifier fires 90/99/100 socket+email+webhook). Notifier is non-throwing — caller (an increment) MUST NOT be broken by a notification failure.
- **Fire-once ledger:** `usage_metering.thresholds_notified` is a JSON array of strings like `"contacts:90"`. The append-if-absent uses an `updateMany` with a `JSON_CONTAINS = false` WHERE clause — a parallel caller's identical write reports `count=0` and skips silently. This is best-effort idempotency; double-firing once under high concurrency is acceptable (the WHERE clause + the Set-based pre-check keep it rare). NEVER drop the in-memory `Set` pre-check.
- **Scope cut for v1 (deliberate):** notifier evaluates `contacts` + `templates` only. `users` is a live count (not a per-period counter), so a future "warning when team adds the Nth member" needs a different trigger path through `TeamService`. Messages / webhook_calls / conversations have no Subscription cap. Don't extend the notifier dimensions without a corresponding cap field.
- `GET /api/billing/usage-warnings` is TenantGuard'd (only useful for active tenants). The sibling `GET /api/billing/account-status` stays JWT-only (a suspended tenant MUST still be able to call it — that endpoint is what surfaces the billing-blocked screen). Don't unify them.
- **MailService** (`common/services/mail.service.ts`) is the shared Resend-or-SMTP sender, used by `LimitNotifierService` for usage + suspension emails. AuthService keeps its own inline sender (verification emails) — NOT migrated in this commit on purpose: the verification path is the most-exercised email flow on the platform; don't disturb it for a refactor.
- **Suspension email** fires once per non-suspended → suspended transition: `SuperAdminService.suspendClient` checks the prior status before sending; `BillingService.enforceCron` only sends inside the loop that actually flips the company. Re-clicking "Suspend" or a second cron run won't re-spam.
- **Banner UX rule:** per-session dismissal only — `sessionStorage` key `${companyId}:${period}:${dim}:${threshold}`. A new session (new tab/window/browser restart) shows the banner again as long as the condition holds. Don't promote this to localStorage — the whole point is the warning recurs until acted on.
- **Navbar pulse:** only renders when `usageSeverity === 'critical'` (≥99%). At `warn` (90%) the chip is static amber; otherwise hidden. Don't make 90% pulse — it would condition users to ignore the animation.
