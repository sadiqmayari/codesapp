...
### [Analytics] old patchwork: KPI strip ignored the date range; "avg response time" wasn't response time; agent stats misattributed
**Error message:** N/A — design-level "doesn't add up."
**Cause:** The old page had four endpoints (`/overview /funnel /agents /conversation-cost`) all reporting different things. **(a)** `/overview`'s KPI strip (Delivered/Read/Reply/Bot-handled) was lifetime + "this month" and took NO date range — but the page had a 7d/30d/90d toggle, so changing the toggle visibly did nothing to the KPIs. **(b)** `agents.avgResponseTimeMin = AVG(TIMESTAMPDIFF(MINUTE, c.created_at, c.updated_at))` is the conversation's age, not response time; it grew the longer a chat lingered. **(c)** `agents.messagesSent` was `SUM(m.direction='outbound')` over `LEFT JOIN messages m ON m.conversation_id = c.id` joined to conversations whose CURRENT `assigned_user_id` was the agent — so every outbound message on a reassigned chat (or broadcast / bot send) attributed to whoever was currently assigned, not who actually composed it. The schema had no `messages.user_id` so it couldn't be done correctly without one. **(d)** Two endpoints used two different definitions of "reply" (overview = unique conversations, funnel = raw inbound count).
**Fix:** Added `messages.user_id` (migration `20260530000000_message_user_id`; `sendMessage`/`sendMedia` write it from `@CurrentUser().userId`). New `GET /api/analytics/dashboard?from&to&granularity&compare` returning everything in ONE range-aware payload (KPIs with previous-period `deltaPct`, trend, funnel, status breakdown, hourly heatmap, agent leaderboard from real `messages.user_id`, top contacts, cost, usage). Real avg first-response latency = AVG of `TIMESTAMPDIFF(SECOND, prev_inbound, this_outbound)` on the same conversation. Cache 60s (was 5 min). Old `/overview /funnel /agents /conversation-cost /usage` routes are kept untouched for backward compat — the new UI just doesn't call them.
