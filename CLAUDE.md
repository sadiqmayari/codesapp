...
│   │   │   └── integrations/
│   │   │       └── shopify/                # …+ ShopifyOrdersController: POST /api/shopify/orders (create-order from chat)
│   │   ├── common/
│   │   │   ├── guards/
│   │   │   │   ├── tenant.guard.ts
│   │   │   │   ├── plan.guard.ts
...
- **Socket auto-refresh:** `socket-context.tsx` `auth` callback is **async** — it decodes the JWT `exp` and calls `/auth/refresh` (single-flight `refreshSocketToken`) before a (re)connect when the token is expiring (≤60s) or expired; `connect_error` also triggers one refresh then a reconnect. Status shows `connecting` (never `disconnected`) while recovering. This is why the live indicator no longer sticks on "offline" after the 15-min access token expires. Do NOT revert the auth callback to the old synchronous `cb({ token: getAccessToken() })`.
...
- **Mobile thread header:** the ~9 action controls are desktop-only (`hidden md:flex`); on mobile they collapse into a `MoreVertical` kebab dropdown (`menuOpen`, outside-click + route-change close). Keeps the header from overflowing the `overflow-hidden` shell off-screen. Any NEW header action must be added to BOTH the desktop row and the mobile menu.
...
- **Canned/quick replies:** new `canned_replies` table + `CannedRepliesModule` → `/api/canned-replies` CRUD, guards `AuthGuard('jwt') + TenantGuard`, tenant-scoped (mirrors SettingsModule). Composer `+` button is a 2-option menu (**Quick reply** → `QuickReplyPicker` inserts the saved body into the composer textarea; **Send template** → existing `TemplatePicker`). Quick replies are company-wide, no Meta involvement (plain text inserted client-side). Only shown inside the 24h window (composer hidden outside it).
- **Shopify create-order from chat:** `POST /api/shopify/orders` (`ShopifyOrdersController`, `@Controller('shopify')` — under `/api`, NOT the root-excluded `integrations/shopify`; guards `AuthGuard('jwt') + TenantGuard`). `ShopifyService.createOrder` = `draftOrderCreate` → `draftOrderComplete(paymentPending:true)` → real **unpaid** order (suits COD/WhatsApp). Uses the company's stored `shopify_admin_token_encrypted` + `shopify_order_configs.{shop_domain,api_version}`. Custom line items via deprecated-but-functional `originalUnitPrice`. Clean 4xx (BadRequest with Shopify userErrors) / 5xx (transport) — never silent. Thread-header `ShoppingBag` button is gated on a once-per-mount `/settings/shopify` `adminTokenSet` fetch (`shopifyReady`). Frontend modal: `create-order-modal.tsx` (repeatable line items + contact prefill).
