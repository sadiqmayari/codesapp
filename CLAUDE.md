...
- **The `ai_agent_company_ids` platform brake was REMOVED (2026-09-22).** The super-admin *AI Agent rollout* panel, the `PlatformSettingService.isAiAgentEnabled` gate in `AiAutoReplyService`, the `AI_AGENT_COMPANY_IDS_KEY` constant, and the settings get/set were all deleted (the `platform_settings` row was dropped too). **The AI agent is now governed ENTIRELY by each tenant's own toggles** — `subscriptions.ai_enabled && companies.ai_enabled` (feature on) + the per-workspace/per-chat `ai_autoreply_enabled` / `conversations.ai_autoreply`. There is no platform-level rollout list anymore. (Removed as the first step of an AI-system redesign.)
- **`ObservabilityService` survives** (it powers `/api/ai/metrics`, `/api/ai/audit/:conversationId` and the super-admin `clients/:id/metrics` mirror), minus the guard-derived counters. Remaining event types: `conversation.handoff`, `order.created`, `order.duplicate_prevented`, `tool.failed`, `image.routed`. `EventStoreService` also survives (the audit timeline + order-idempotency depend on it).
...
- **The ENGAGEMENT ENGINE was DELETED in full (Sept-2026)** — `modules/engagement/` (work items, per-type FSMs, RouterService, ToldLedgerService), its cron controller, the `engagement_engine` feature flag + `engagementModeFor`, the `ENGAGEMENT_ENGINE_*` platform keys, the super-admin UI section, and the work-item scoping inside `AiService.loadTranscript`/`buildAgentContext`. Migration `20260903000000_remove_engagement_engine` drops `work_items`, `told_ledger`, `messages.work_item_id`, `messages.seq`, `companies.engagement_mode`, the two platform settings, and purges ~144k engagement `events` rows. It was enabled for ONE tenant and last routed a message 2026-08-17 — dormant ever since, because it runs inside the auto-reply path and that tenant has `ai_autoreply_enabled = 0`. The specialist is now always chosen by `applyTopicManager` (the topic/episode heuristic); there is no authoritative-routing branch.
- **The transactional `outbox` was deleted with it** — `OutboxService`/`OutboxSenderService`/the `outbox` table. It had no caller anywhere and 0 rows in production. Don't mistake it for a working send-reliability mechanism; there isn't one.
- **Job-queue health moved out:** `GET /cron/engagement/metrics` → **`GET /cron/health/metrics`** (`QueueHealthService`, `modules/cron/`). Same queue-depth / oldest-pending / dead-letter snapshot minus the work-item parts. Update `/etc/cron.d/codesapp`; the old `/cron/engagement/*` entries (metrics AND sla-sweep) now 404.
