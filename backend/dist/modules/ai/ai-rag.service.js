...
let AiRagService = AiRagService_1 = class AiRagService {
    ...
    async status(companyId) {
        const rows = await this.prisma.$queryRaw `SELECT source_type, COUNT(*) AS n, MAX(updated_at) AS last
        ...
        for (const r of rows) {
            const n = Number(r.n);
            if (r.source_type === 'product')
                products = n;
            else if (r.source_type === 'policy')
                policies = n;
            if (r.last && (!last || r.last > last))
                last = r.last;
        }
        return {
